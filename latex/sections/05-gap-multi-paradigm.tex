\subsection{Gap 3: Limited Computational Paradigm Support}
\label{sec:multiparadigm}

The in vivo data integration challenge from the previous section illustrates a deeper architectural limitation: agents are not missing specific tools so much as they lack the capacity to orchestrate diverse computational paradigms. "Multi-agent" systems in drug discovery have multiple LLM-based agents collaborating \citep{seal2025aiagents}. PharmAgents deploys specialized agents for target identification and synthesis. MADD coordinates molecular design and docking \citep{madd2025}. BioPlanner benchmarks LLM-driven protocol planning \citep{bioplanner2023}, and ChemToolAgent evaluates chemistry tool integration \citep{chemtoolagent2024}. The pattern: an orchestrator LLM decomposes queries, delegates to modules, and synthesizes outputs.
...
\subsubsection{The LLM-as-Orchestrator Limitation}

LLM-centric design treats the language model as central coordinator, invoking external tools via APIs. ChemCrow calls RDKit and PubChem \citep{bran2024chemcrow}. ChatInvent supports molecular design and synthesis planning \citep{he2026chatinvent}. This works for text-based reasoning and stateless tools.
...
\subsubsection{The Missing Paradigms}

Absent paradigms define modern drug discovery \citep{schneider2020generative}: supervised learning (bioactivity prediction \citep{stokes2020ml}, toxicity modeling), unsupervised learning (chemical space clustering), generative modeling (de novo design \citep{grisoni2021synthesis}), reinforcement learning (multi-objective optimization \citep{tan2022rl}), simulation (binding kinetics), and optimization (experimental design).
...
\hline
\multicolumn{4}{l}{\footnotesize $^*$Runtime includes experimental turnaround, not just computation.} \\
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig4-multi-paradigm-architecture.jpeg}
...
Effective interaction would be batch-mode, not chat. Bottlenecks are orchestrating end-to-end analyses, not formulating queries. Small biotechs managing dozens of projects require automation: "Analyze eight RNA-seq samples, perform differential expression and pathway enrichment, generate report," with agents intervening only for human decisions (conflicting pathway interpretations).
