...
Peptide stability illustrates the trap. The stability-activity trade-offs described in \S\ref{sec:small-molecule} create a multidimensional decision space where the balance depends on route of administration, therapeutic window, and development timeline.
...
Bayesian optimization handles uncertainty via Gaussian processes providing means and variances. Acquisition functions balance exploitation and exploration (see \S\ref{sec:smallbiotech} for detailed discussion). Visualizing uncertainty to guide exploration versus exploitation is a key capability gap.
